        ...
        for i in range(len(W)):
            ...
            for j in range(len(W[i])):
                ...
                if r5<=0.7:
                    ...
                    temp.append((5/(1-lambbda*r3-0.5*r4*(1-lambbda)))*((4*W[i-1][j-1]+2*W[i-2][j-2])/10)*(1-lambbda*r3-0.5*r4*(1-lambbda)+lambbda*r3*Wbest[j]+0.5*r4*(1-lambbda)*Wbest[j]))  # updated equation
            W_.append(temp)
        return W_
